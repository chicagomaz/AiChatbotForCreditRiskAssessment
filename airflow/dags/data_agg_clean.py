...
# Fixed: null value handling
    ...
    df["FLAG_CREDIT_BUREAU_MISSING"] = (
        df[credit_bureau_cols].isnull().any(axis=1).astype(int)
    )
    ...
    building_cols = [
        ...
        if "AVG" in col or "MODE" in col or "MEDI" in col
    ]
    df["FLAG_BUILDING_INFO_MISSING"] = (
        df[building_cols].isnull().any(axis=1).astype(int)
    )
